...
3. **Explanations**: Concept Lesson checks and Worked Example steps use `How to Solve:` to continue the calculation or reasoning required at that step. For conceptual or recognition steps without procedural work, explain which prompt detail, definition, notation, or representation supports the answer. Other answer-bearing items use `Solution:` and `Why it works:`; multiple choice items outside Concept Lessons and Worked Examples also explain why each plausible distractor fails. Recall Drill items and Glossary drills use a general explanation of why the answer is correct and do not require these labels or separate distractor feedback.
